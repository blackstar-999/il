...
import com.blackstar.blackstar.messages.BlackSavePreferences;
...
public class MessagesController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public boolean isPremiumUser(TLRPC.User currentUser) {
        return !premiumLocked && (currentUser.premium || currentUser.id == getUserConfig().getClientUserId() && BlackConfig.localPremium);
    }
...
//            editor.putInt("maxPinnedDialogsCount", maxPinnedDialogsCount);
    ...
    public void deleteMessages(ArrayList<Integer> messages, ArrayList<Long> randoms, TLRPC.EncryptedChat encryptedChat, long dialogId, boolean forAll, boolean scheduled, boolean cacheOnly, long taskId, TLObject taskRequest) {
        ...
        // --- Black StaR hook
        if (!scheduled && BlackConfig.saveDeletedMessages) {
            if (DialogObject.isEncryptedDialog(dialogId) && messages != null && !messages.isEmpty()) { // process TTL messages from secrets
                for (int a = 0; a < messages.size(); a++) {
                    ...
                    if (BlackState.isDeletePermitted(dialogId, id)) {
                        continue;
                    }
                    ...
                    if (obj != null) {
                        var prefs = new BlackSavePreferences(obj.messageOwner, currentAccount);
                        prefs.setDialogId(dialogId);
                        BlackMessagesController.getInstance().onMessageDeleted(prefs);
                    }
                }

                AndroidUtilities.runOnUIThread(() -> {
                    // invalidating views
                    getNotificationCenter().postNotificationName(BlackConstants.MESSAGES_DELETED_NOTIFICATION, dialogId, messages);
                });
                ...
                for (var msgId : messages) {
                    if (BlackState.isDeletePermitted(dialogId, msgId)) {
                        continue;
                    }
                    ...
                    if (msg != null) {
                        if (msg.ttl > 0 || msg.ttl_period > 0) {
                            ...
                            var prefs = new BlackSavePreferences(msg, currentAccount);
                            prefs.setDialogId(dialogId);
                            BlackMessagesController.getInstance().onMessageDeleted(prefs);
                        }
                    }
                ...
                AndroidUtilities.runOnUIThread(() -> {
                    // invalidating views
                    getNotificationCenter().postNotificationName(BlackConstants.MESSAGES_DELETED_NOTIFICATION, dialogId, invalidate);
                });
            } else if ((messages != null && !messages.isEmpty())) { // process manual deletion of deleted messages
                var ayuMessagesController = BlackMessagesController.getInstance();
...
//                            if (newMsg == null && oldMs.getId() > 0 || newMsg != null && newMsg.messageOwner.date > oldMsg.messageOwner.date)
    ...
    public void markMessageAsRead(long dialogId, long randomId, int ttl) {
        ...
        if (ttl > 0 && !BlackConfig.saveDeletedMessages) {
            int time = getConnectionsManager().getCurrentTime();
...
//                    }
    ...
    private void checkChannelError(String text, long channelId) {
        if (BlackConfig.saveDeletedMessages) {
            return;
        }
    ...
    public boolean processUpdateArray(ArrayList<TLRPC.Update> updates, ArrayList<TLRPC.User> usersArr, ArrayList<TLRPC.Chat> chatsArr, boolean fromGetDifference, int date) {
        ...
        // --- Black StaR request hook
        if (BlackConfig.saveDeletedMessages && deletedMessages != null) {
            ...
            for (int a = 0, size = deletedMessages.size(); a < size; a++) {
                ...
                for (var dialogId : dialogIds) {
                    for (var msgId : messageIds) {
                        ...
                        var prefs = new BlackSavePreferences(msg, currentAccount, dialogId, topicId, msgId, currentTimeS);
                        ayuMessagesController.onMessageDeleted(prefs);
                    }

                    AndroidUtilities.runOnUIThread(() -> {
                        // invalidating views
                        getNotificationCenter().postNotificationName(BlackConstants.MESSAGES_DELETED_NOTIFICATION, dialogId, messageIds);
                    });
                }
            }
        }
        ...
        getMessagesStorage().getStorageQueue().postRunnable(() -> AndroidUtilities.runOnUIThread(() -> {
            ...
            if (deletedMessagesFinal != null) { // --- Black StaR: don't notify that messages were deleted; already handled by MESSAGES_DELETED_NOTIFICATION
        ...
        getConnectionsManager().sendRequest(req, (response, error) -> {
            ...
            if (response != null) {
                ...
                if (res.peers.isEmpty()) {
                    ...
                    if (!BlackConfig.localPremium) { // для даунов, потом мб уберу
