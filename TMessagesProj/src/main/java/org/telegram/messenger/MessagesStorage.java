...
public class MessagesStorage extends BaseController {
    ...
    public void openDatabase(int openTries) {
        ...
        try {
            ...
            database.executeFast("PRAGMA journal_mode = " + BlackConfig.getWALMode()).stepThis().dispose();
            ...
            if (createTable) {
                ...
                try {
                    database.executeFast("CREATE TABLE IF NOT EXISTS dialog_filter_blackstar(id INTEGER PRIMARY KEY, ord INTEGER, unread_count INTEGER, flags INTEGER, title TEXT, emoticon TEXT)").stepThis().dispose();
                } catch (Exception e) {
                    FileLog.e(e);
                }
    ...
    public static void createTables(SQLiteDatabase database) throws SQLiteException {
        ...
        database.executeFast("CREATE TABLE dialog_filter_blackstar(id INTEGER PRIMARY KEY, ord INTEGER, unread_count INTEGER, flags INTEGER, title TEXT, emoticon TEXT)").stepThis().dispose();
    ...
    private void loadDialogFilters() {
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                filtersCursor = database.queryFinalized("SELECT id, ord, unread_count, flags, title, emoticon FROM dialog_filter_blackstar WHERE 1");
                ...
                if (!hasDefaultFilter) {
                    ...
                    state = database.executeFast("REPLACE INTO dialog_filter_blackstar VALUES(?, ?, ?, ?, ?, ?)");
    ...
    private void deleteDialogFilterInternal(MessagesController.DialogFilter filter) {
        try {
            ...
            database.executeFast("DELETE FROM dialog_filter_blackstar WHERE id = " + filter.id).stepThis().dispose();
            database.executeFast("DELETE FROM dialog_filter_ep WHERE id = " + filter.id).stepThis().dispose();
            database.executeFast("DELETE FROM dialog_filter_pin_v2 WHERE id = " + filter.id).stepThis().dispose();
        } catch (Exception e) {
            checkSQLException(e);
    ...
    public void saveDialogFiltersOrderInternal() {
        SQLitePreparedStatement state = null;
        try {
            state = database.executeFast("UPDATE dialog_filter_blackstar SET ord = ?, flags = ? WHERE id = ?");
            for (int a = 0, N = dialogFilters.size(); a < N; a++) {
                MessagesController.DialogFilter filter = dialogFilters.get(a);
                state.requery();
                    ...
                    if (data != null) {
                        ...
                        if (message.media != null) {
                            if (!addFilesToDelete(message, filesToDelete, idsToDelete, namesToDelete, true)) {
                                ...
                                // --- Black StaR hook
                                if (BlackConfig.saveMessagesHistory) {
                                    var prefs = new BlackSavePreferences(message, currentAccount);
                                    prefs.setDialogId(dialogId);
                                    BlackMessagesController.getInstance().onMessageEditedForce(prefs);
                                }
                                // --- Black StaR hook
                            ...
                            if (exist = cursor.next()) {
                                ...
                                if (data != null) {
                                    ...
                                    if (message.from_id != null && (!oldMessage.message.equals(message.message) || !sameMedia)) {
                                        var prefs = new BlackSavePreferences(oldMessage, currentAccount);
                                        prefs.setDialogId(dialogId);
                                        BlackMessagesController.getInstance().onMessageEdited(prefs, message);
                                    }
                                    if (!sameMedia) {
