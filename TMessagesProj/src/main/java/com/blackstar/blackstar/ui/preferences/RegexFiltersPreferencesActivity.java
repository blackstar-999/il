...
import com.blackstar.blackstar.ui.preferences.utils.RegexFilterPopup;
...
public class RegexFiltersPreferencesActivity extends BasePreferencesActivity {
    ...
    protected void updateRowsId() {
        ...
        filters = BlackConfig.getRegexFilters();
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position > filtersHeaderRow && filtersHeaderRow != -1) {
            ...
            BlackConfig.editor.putBoolean("regexFiltersInChats", BlackConfig.regexFiltersInChats ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.regexFiltersInChats);
        } else if (position == caseInsensitiveRow) {
            BlackConfig.editor.putBoolean("regexFiltersCaseInsensitive", BlackConfig.regexFiltersCaseInsensitive ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.regexFiltersCaseInsensitive);
            ...
            switch (holder.getItemViewType()) {
                ...
                case 5:
                    ...
                    if (position == enableInChatsRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.RegexFiltersEnableInChats), BlackConfig.regexFiltersInChats, true);
                    } else if (position == caseInsensitiveRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.RegexFiltersCaseInsensitive), BlackConfig.regexFiltersCaseInsensitive, false);
                    }
                    break;
            }
        }
