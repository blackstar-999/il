...
import com.blackstar.blackstar.messages.BlackMessagesController;
import com.blackstar.blackstar.sync.BlackSyncState;
import com.blackstar.blackstar.ui.preferences.utils.BlackUi;
import com.blackstar.blackstar.utils.BlackState;
...
public class BlackStaRPreferencesActivity extends BasePreferencesActivity implements NotificationCenter.NotificationCenterDelegate {
    ...
    private int clearBlackDatabaseBtnRow;
    ...
    protected void updateRowsId() {
        ...
        clearBlackDatabaseBtnRow = newRow();
    ...
    public boolean onFragmentCreate() {
        ...
        NotificationCenter.getInstance(UserConfig.selectedAccount).addObserver(this, BlackConstants.MESSAGES_DELETED_NOTIFICATION);
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        if (id == BlackConstants.MESSAGES_DELETED_NOTIFICATION) {
            // recalculate database size
            if (listAdapter != null) {
                listAdapter.notifyItemChanged(clearBlackDatabaseBtnRow);
            }
        } else if (id == BlackConstants.AYUSYNC_STATE_CHANGED) {
            if (listAdapter != null) {
    ...
    public void onFragmentDestroy() {
        ...
        NotificationCenter.getInstance(UserConfig.selectedAccount).removeObserver(this, BlackConstants.MESSAGES_DELETED_NOTIFICATION);
    ...
    private void updateGhostViews() {
        var isActive = BlackConfig.isGhostModeActive();
    ...
    private void toggleLocalPremium() {
        var newState = !BlackConfig.localPremium;

        BlackConfig.editor.putBoolean("localPremium", BlackConfig.localPremium = newState).apply();
        listAdapter.notifyItemChanged(localPremiumRow, BlackConfig.localPremium);

        getMessagesController().updatePremium(BlackConfig.localPremium);
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position == ghostModeToggleRow) {
            ...
            BlackConfig.editor.putBoolean("sendReadPackets", BlackConfig.sendReadPackets ^= true).apply();
            ((CheckBoxCell) view).setChecked(BlackConfig.sendReadPackets, true);

            BlackState.setAllowReadPacket(false, -1);
            updateGhostViews();
        } else if (position == sendOnlinePacketsRow) {
            BlackConfig.editor.putBoolean("sendOnlinePackets", BlackConfig.sendOnlinePackets ^= true).apply();
            ((CheckBoxCell) view).setChecked(BlackConfig.sendOnlinePackets, true);

            updateGhostViews();
        } else if (position == sendUploadProgressRow) {
            BlackConfig.editor.putBoolean("sendUploadProgress", BlackConfig.sendUploadProgress ^= true).apply();
            ((CheckBoxCell) view).setChecked(BlackConfig.sendUploadProgress, true);

            updateGhostViews();
        } else if (position == sendOfflinePacketAfterOnlineRow) {
            BlackConfig.editor.putBoolean("sendOfflinePacketAfterOnline", BlackConfig.sendOfflinePacketAfterOnline ^= true).apply();
            ((CheckBoxCell) view).setChecked(BlackConfig.sendOfflinePacketAfterOnline, true);

            updateGhostViews();
        } else if (position == markReadAfterSendRow) {
            BlackConfig.editor.putBoolean("markReadAfterSend", BlackConfig.markReadAfterSend ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.markReadAfterSend);

            BlackState.setAllowReadPacket(false, -1);

            if (BlackConfig.markReadAfterSend && BlackConfig.useScheduledMessages) {
                BlackConfig.editor.putBoolean("useScheduledMessages", BlackConfig.useScheduledMessages ^= true).apply();

                listAdapter.notifyItemChanged(useScheduledMessagesRow, false);
            }
        } else if (position == useScheduledMessagesRow) {
            BlackConfig.editor.putBoolean("useScheduledMessages", BlackConfig.useScheduledMessages ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.useScheduledMessages);

            BlackState.setAutomaticallyScheduled(false, -1);

            if (BlackConfig.useScheduledMessages && BlackConfig.markReadAfterSend) {
                BlackConfig.editor.putBoolean("markReadAfterSend", BlackConfig.markReadAfterSend ^= true).apply();

                listAdapter.notifyItemChanged(markReadAfterSendRow, false);
            }
        } else if (position == saveDeletedMessagesRow) {
            BlackConfig.editor.putBoolean("saveDeletedMessages", BlackConfig.saveDeletedMessages ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.saveDeletedMessages);
        } else if (position == saveMessagesHistoryRow) {
            BlackConfig.editor.putBoolean("saveMessagesHistory", BlackConfig.saveMessagesHistory ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.saveMessagesHistory);
            ...
            BlackConfig.editor.putBoolean("keepAliveService", BlackConfig.keepAliveService ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.keepAliveService);
        } else if (position == disableAdsRow) {
            BlackConfig.editor.putBoolean("disableAds", BlackConfig.disableAds ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.disableAds);
        } else if (position == localPremiumRow) {
            toggleLocalPremium();
        } else if (position == filtersRow) {
            NotificationsCheckCell checkCell = (NotificationsCheckCell) view;
            if (LocaleController.isRTL && x <= AndroidUtilities.dp(76) || !LocaleController.isRTL && x >= view.getMeasuredWidth() - AndroidUtilities.dp(76)) {
                BlackConfig.editor.putBoolean("regexFiltersEnabled", BlackConfig.regexFiltersEnabled ^= true).apply();
                checkCell.setChecked(BlackConfig.regexFiltersEnabled, 0);
            } else {
                presentFragment(new RegexFiltersPreferencesActivity());
            }
        } else if (position == showGhostToggleInDrawerRow) {
            BlackConfig.editor.putBoolean("showGhostToggleInDrawer", BlackConfig.showGhostToggleInDrawer ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.showGhostToggleInDrawer);

            NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.mainUserInfoChanged);
        } else if (position == showKillButtonInDrawerRow) {
            BlackConfig.editor.putBoolean("showKillButtonInDrawer", BlackConfig.showKillButtonInDrawer ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.showKillButtonInDrawer);
            ...
            BlackUi.spawnEditBox(
                    ...
                    BlackConfig::getDeletedMark,
                    "deletedMarkText",
                    BlackConstants.DEFAULT_DELETED_MARK
            );
            ...
            BlackUi.spawnEditBox(
                    ...
                    BlackConfig::getEditedMark,
                    "editedMarkText",
                    LocaleController.getString("EditedMessage", R.string.EditedMessage) // don't remove key
            );
        } else if (position == ayuSyncStatusBtnRow) {
            presentFragment(new BlackSyncPreferencesActivity());
        } else if (position == WALModeRow) {
            BlackConfig.editor.putBoolean("WALMode", BlackConfig.WALMode ^= true).apply();
            ((TextCheckCell) view).setChecked(BlackConfig.WALMode);
        } else if (position == clearBlackDatabaseBtnRow) {
            BlackMessagesController.getInstance().clean();

            // reset size
            ((TextCell) view).setValue("…");

            BulletinFactory.of(this).createSimpleBulletin(R.raw.info, LocaleController.getString(R.string.ClearBlackDatabaseNotification)).show();
    ...
    protected String getTitle() {
        return LocaleController.getString(R.string.BlackPreferences);
    }
    ...
    private int getGhostModeSelectedCount() {
        int count = 0;
        if (!BlackConfig.sendReadPackets) count++;
        if (!BlackConfig.sendOnlinePackets) count++;
        if (!BlackConfig.sendUploadProgress) count++;
        if (BlackConfig.sendOfflinePacketAfterOnline) count++;
            ...
            switch (holder.getItemViewType()) {
                ...
                case 2:
                    ...
                    if (position == messageSavingBtnRow) {
                        ...
                        textCell.setTextAndValue(LocaleController.getString(R.string.DeletedMarkText), BlackConfig.getDeletedMark(), true);
                    } else if (position == editedMarkTextRow) {
                        textCell.setTextAndValue(LocaleController.getString(R.string.EditedMarkText), BlackConfig.getEditedMark(), true);
                    } else if (position == ayuSyncStatusBtnRow) {
                        var status = BlackSyncState.getConnectionStateString();

                        textCell.setTextAndValue(LocaleController.getString(R.string.BlackSyncStatusTitle), status, false);
                    } else if (position == clearBlackDatabaseBtnRow) {
                        var file = ApplicationLoader.applicationContext.getDatabasePath(BlackConstants.AYU_DATABASE);
                        var size = file.exists() ? file.length() : 0;

                        textCell.setTextAndValueAndIcon(LocaleController.getString(R.string.ClearBlackDatabase), AndroidUtilities.formatFileSize(size), R.drawable.msg_clear_solar, true);
                ...
                case 3:
                    ...
                    if (position == ghostEssentialsHeaderRow) {
                        ...
                        headerCell.setText(LocaleController.getString(R.string.BlackSyncHeader));
                ...
                case 5:
                    ...
                    if (position == markReadAfterSendRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.MarkReadAfterSend), BlackConfig.markReadAfterSend, true);
                    } else if (position == useScheduledMessagesRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.UseScheduledMessages), BlackConfig.useScheduledMessages, false);
                    } else if (position == saveDeletedMessagesRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.SaveDeletedMessages), BlackConfig.saveDeletedMessages, true);
                    } else if (position == saveMessagesHistoryRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.SaveMessagesHistory), BlackConfig.saveMessagesHistory, false);
                    } else if (position == keepAliveServiceRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.KeepAliveService), BlackConfig.keepAliveService, true);
                    } else if (position == disableAdsRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.DisableAds), BlackConfig.disableAds, true);
                    } else if (position == localPremiumRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.LocalPremium) + " β", BlackConfig.localPremium, true);
                    } else if (position == showGhostToggleInDrawerRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.ShowGhostToggleInDrawer), BlackConfig.showGhostToggleInDrawer, true);
                    } else if (position == showKillButtonInDrawerRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.ShowKllButtonInDrawer), BlackConfig.showKillButtonInDrawer, false);
                    } else if (position == WALModeRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.WALMode), BlackConfig.WALMode, false);
                    }
                    break;
                case 18:
                    ...
                    if (position == ghostModeToggleRow) {
                        ...
                        checkCell.setTextAndCheck(LocaleController.getString(R.string.GhostModeToggle), BlackConfig.isGhostModeActive(), true, true);
                        checkCell.setCollapseArrow(String.format(Locale.US, "%d/4", selectedCount), !ghostModeMenuExpanded, () -> {
                            BlackConfig.toggleGhostMode();
                            updateGhostViews();
                        });
                    }
                ...
                case 19:
                    ...
                    if (position == sendReadPacketsRow) {
                        checkBoxCell.setText(LocaleController.getString(R.string.DontSendReadPackets), "", !BlackConfig.sendReadPackets, true, true);
                    } else if (position == sendOnlinePacketsRow) {
                        checkBoxCell.setText(LocaleController.getString(R.string.DontSendOnlinePackets), "", !BlackConfig.sendOnlinePackets, true, true);
                    } else if (position == sendUploadProgressRow) {
                        checkBoxCell.setText(LocaleController.getString(R.string.DontSendUploadProgress), "", !BlackConfig.sendUploadProgress, true, true);
                    } else if (position == sendOfflinePacketAfterOnlineRow) {
                        checkBoxCell.setText(LocaleController.getString(R.string.SendOfflinePacketAfterOnline), "", BlackConfig.sendOfflinePacketAfterOnline, true, true);
                    }
                ...
                case TOGGLE_BUTTON_VIEW:
                    ...
                    if (position == filtersRow) {
                        var count = BlackConfig.getRegexFilters().size();
                        notificationsCheckCell.setTextAndValueAndCheck(LocaleController.getString(R.string.RegexFilters), count + " " + LocaleController.getString(R.string.RegexFiltersAmount), BlackConfig.regexFiltersEnabled, false);
                    }
                    break;
            }
        }
            ...
            if (
                ...
                return 1;
                    ...
                    position == messageSavingBtnRow ||
                            ...
                            position == clearBlackDatabaseBtnRow ||
