...
import com.blackstar.blackstar.easy.BlackEasyUtils;
...
public class BlackForwarder {
    public static boolean isFullBlackForwardsNeeded(int currentAccount, ArrayList<MessageObject> messages) {
    ...
    public static boolean isBlackForwardNeeded(ArrayList<MessageObject> messages) {
        for (var message : messages) {
            if (isBlackForwardNeeded(message)) {
                return true;
            }
        }
    ...
    private static boolean isBlackForwardNeeded(MessageObject message) {
    ...
    public static void intelligentForward(int currentAccount, ArrayList<MessageObject> messages, long peer, boolean forwardFromMyName, boolean hideCaption, boolean notify, int scheduleDate, MessageObject replyToTopMsg) {
        ...
        var currentBatch = new ForwardBatch(isBlackForwardNeeded(messages.get(0)), currentArray);

        for (var message : messages) {
            if (isBlackForwardNeeded(message) != currentBatch.isBlackForwardNeeded) {
                batches.add(currentBatch);

                currentArray = new ArrayList<>();
                currentBatch = new ForwardBatch(isBlackForwardNeeded(message), currentArray);
            }
        ...
        for (var batch : batches) {
            if (batch.isBlackForwardNeeded) {
                ...
                BlackEasyUtils.forwardMessagesSync(currentAccount, batch.messages, peer, forwardFromMyName, hideCaption, notify, scheduleDate, replyToTopMsg);
            }
        }
    }
    ...
    public static void forwardMessages(int currentAccount, ArrayList<MessageObject> messages, long peer, boolean forwardFromMyName, boolean hideCaption, boolean notify, int scheduleDate, MessageObject replyToTopMsg) {
        ...
        var fullNoforwards = isFullBlackForwardsNeeded(currentAccount, messages);

        for (var message : messages) {
            if (fullNoforwards || message.messageOwner.ayuNoforwards || message.messageOwner.ayuDeleted) {
                if (BlackUtils.isMediaDownloadable(message, false)) {
        ...
        if (!toBeDownloaded.isEmpty()) {
            BlackEasyUtils.loadDocumentsSync(currentAccount, toBeDownloaded);
        }

        for (var message : messages) {
            var textExtracted = ChatUtils.getMessageText(message, null);
            var text = textExtracted == null ? "" : textExtracted.toString();
            var mediaDownloadable = BlackUtils.isMediaDownloadable(message, false);
            ...
            if (!mediaDownloadable) {
                BlackEasyUtils.sendTextMessageSync(
                        currentAccount,
                        text,
                        peer,
                        null,
                ...
                BlackEasyUtils.sendDocumentMessageSync(
                        currentAccount,
                        newDoc,
                ...
                BlackEasyUtils.sendPhotoMessageSync(
                        currentAccount,
                        newPhoto,
                        messagePath.getAbsolutePath(),
                        peer,
                ...
                Log.w("BlackStaR", "Unsupported message type: " + message.messageOwner);
            }

            Log.w("BlackStaR", "Message forwarded");
        }

        Log.w("BlackStaR", "All messages forwarded");
    }
    ...
    private static class ForwardBatch {
        public final boolean isBlackForwardNeeded;
        public final ArrayList<MessageObject> messages;

        public ForwardBatch(boolean isBlackForwardNeeded, ArrayList<MessageObject> messages) {
            this.isBlackForwardNeeded = isBlackForwardNeeded;
